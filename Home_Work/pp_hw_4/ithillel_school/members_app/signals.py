...
def course_pre_save(sender, instance, **kwargs):
    print(f"[pre_save][Course] Готується до збереження: {instance.name}")


@receiver(post_save, sender=Course)
def course_post_save(sender, instance, created, **kwargs):
    if created:
        print(f"[post_save][Course] Створено новий курс: {instance.name}")
    else:
        print(f"[post_save][Course] Курс оновлено: {instance.name}")


@receiver(pre_delete, sender=Course)
def course_pre_delete(sender, instance, **kwargs):
    print(f"[pre_delete][Course] Курс буде видалено: {instance.name}")


@receiver(post_delete, sender=Course)
def course_post_delete(sender, instance, **kwargs):
    print(f"[post_delete][Course] Курс видалено: {instance.name}")
